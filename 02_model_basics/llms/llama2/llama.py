...
class Attention(nn.Module):
    ...
    def compute_query_key_value_scores(self,
        ...
        attn_probs = self.attn_dropout(F.softmax(attn_score, dim=-1))
    ...
    def generate(self, idx, max_new_tokens, temperature=1.0, epsilon=0.05):
        ...
        for _ in range(max_new_tokens):
            ...
            if temperature == 0.0:
                ...
                '''
                temp_scaled_logits = F.softmax(logits / temperature, dim=-1)
                masked_logits = torch.where(
                    temp_scaled_logits >= epsilon, 
                    temp_scaled_logits, 
                    torch.zeros_like(temp_scaled_logits)
                )
                probs_sum = masked_logits.sum(dim=-1, keepdim=True)
                probs = masked_logits / (probs_sum + 1e-8)
